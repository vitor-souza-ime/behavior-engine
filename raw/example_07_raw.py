...
# ──────────────────────────────────────────────
# 7. Intermediário — duas zonas de distância (SEM FRAMEWORK)
# ──────────────────────────────────────────────
...
try:
    while True:
        ...
        # Regra 1: reject se pessoa muito perto
        if min_person_distance is not None and min_person_distance <= REJECT_THRESHOLD_METERS:
            if (current_time - last_action_time_reject) > COOLDOWN:
                send_action_reject()
                last_action_time_reject = current_time

        # Regra 2: shake hand se pessoa em distância média
        elif min_person_distance is not None and min_person_distance <= SHAKE_HAND_THRESHOLD_METERS:
