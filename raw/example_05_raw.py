...
# ──────────────────────────────────────────────
# 5. Intermediário — aperto de mão em distância média (SEM FRAMEWORK)
# ──────────────────────────────────────────────
...
SHAKE_HAND_THRESHOLD_METERS = 1.0
...
try:
    while True:
        ...
        person_within_range = False
        for box in results.boxes:
            cls_name = model.names[int(box.cls)]
            if cls_name != "person":
                continue
            distance = get_object_distance(depth_frame, box, depth_scale)
            if distance is not None and distance <= SHAKE_HAND_THRESHOLD_METERS:
                person_within_range = True
                break

        current_time = time.time()
        if person_within_range and (current_time - last_action_time) > COOLDOWN:
